    ...
    <div className="max-w-md mx-auto">
      <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-black/5 shadow-warm">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-heading font-bold mb-1">Masuk</h1>
          <p className="text-sm text-text-secondary">Lanjutkan proses booking Anda.</p>
        </div>

        <Button type="button" variant="outline" onClick={handleGoogle} disabled={loading} className="w-full rounded-full border-2 mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {error && <div className="text-sm text-red bg-red/10 border border-red/20 rounded-2xl px-4 py-3">{error}</div>}
          <Button type="submit" disabled={loading} className="w-full rounded-full shadow-warm" size="lg">
            {loading ? <><Loader2 className="animate-spin mr-2" size={18} /> Memproses…</> : <><LogIn size={18} className="mr-2" /> Masuk</>}
          </Button>
        </form>

        <p className="text-xs text-text-secondary text-center mt-4">
          Belum punya akun?{" "}
          <Link to={`/signup?next=${encodeURIComponent(next)}`} className="text-primary font-semibold hover:underline">Daftar</Link>
        </p>
      </div>
    </div>
  );
